...
namespace Services.AssetsAddressables
{
    public class AssetsAddressablesProvider : IAssetsAddressablesProvider
    {
        ...
        private async Task<T> RunWinCacheOnComplete<T>(AsyncOperationHandle<T> handle, string key) where T : Object
        {
            handle.Completed += operation =>
            {
                if (operation.Status == AsyncOperationStatus.Succeeded)
                {
                    AddHandle<T>(key, operation);
                    _completedOperations[key] = operation;
                }
            };
